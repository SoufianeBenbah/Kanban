...
public class TaskServiceImpl implements TaskService {
	...
	public Task changeTaskStatus(Task task, TaskStatus targetStatus) {
		...
		changeLog.setOccurred(LocalDateTime.now());
	...
	@Transactional(readOnly = true)
	public TaskStatus getTargetStatusForMoveRight(TaskStatus status) {
		...
		if (status != null) {
			if (status.equals(todoStatus)) {
				...
				throw new IllegalStateException("Cant move task it s already done !");
			}
		}
		else {
	...
	@Transactional(readOnly = true)
	public TaskStatus getTargetStatusForMoveLeft(TaskStatus status) {
		...
		TaskStatus result = todoStatus;
		
		if (status != null) {
			if (status.equals(todoStatus)) {
				throw new IllegalStateException("It is already a TO DO");
			}
			else if (status.equals(doingStatus)) {
				result = todoStatus;
			}
	...
	public void deleteTask(Long id) {
		...
		Task task = this.taskRepository.save(taskRepository.getOne(id));
		
		List<ChangeLog> changeLogs = this.changeLogRepository.findByTaskId(id);
		
		for (ChangeLog changeLog : changeLogs){ this.changeLogRepository.delete(changeLog); }
